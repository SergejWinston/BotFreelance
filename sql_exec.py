...
def check(table: str, columns: str, search_text, log=1) -> list: 
    ...
    conn.close() 
    if log == 1:
        log_tg.log(3, msg=f"Возращаем из таблицы {table}, строчку с поисковыми значениями: {columns} -> {search_text}" + Fore.RESET)
    else:
        pass
...
def check_contains(table: str, columns: str, search_text: str) -> list: 
    ...
    results = cursor.fetchall() 
    conn.close() 
    log_tg.log(3, msg=f"Возращаем из таблицы {table}, строчку с поисковыми значениями содержащими: {columns} -> {search_text}" + Fore.RESET)
    return [row for row in results]
...
def insert(table: str, columns, values) -> None:
    ...
    log_tg.log(3, msg=f" ".join(columns.split(",")) + Fore.RESET)
    log_tg.log(3, msg=f" ".join(values.split(",")) + Fore.RESET)
...
def set_state(id: int, state) -> None:
    ...
    conn.commit()  
    conn.close() 
    log_tg.log(3, msg=f"Установили статус '{state}' для пользователя {id}" + Fore.RESET)
    return
...
def delete_chat(id: int) -> None:
    ...
    conn.commit()  
    conn.close() 
    log_tg.log(3, msg=f"Удалили строчку из таблицы 'chats' с значением 'chat_id' -> {id}" + Fore.RESET)
    return
...
def remove_line(table, column, value):
    ...
    conn.commit()  
    conn.close() 
    log_tg.log(3, msg=f"Удалили строчку из таблицы '{table}' с значением '{column}' -> {value}" + Fore.RESET)
    return
...
def get_pos_line(table: str, pos: int) -> int:
    ...
    results = cursor.fetchall() 
    conn.close() 
    log_tg.log(3, msg=f"Получаем cуществует ли строчка из таблицы '{table}' с индексом {pos}" + Fore.RESET)
    return len(results)
...
def get_pos_line_result(table: str, pos: int) -> list:
    ...
    results = cursor.fetchall() 
    conn.close() 
    log_tg.log(3, msg=f"Получаем строчку из таблицы '{table}' с индексом {pos}" + Fore.RESET)
    return results
...
def set(table: str, search_column: str, search_value, change_column: str, change_value) -> None:
    ...
    conn.commit()  
    conn.close() 
    log_tg.log(3, msg=f"Задаем '{change_column}' -> '{change_value}' в таблице '{table}', используя поисковой запрос: {search_column} -> {search_value}" + Fore.RESET)
    return
...
def set_null(table: str, search_column: str, search_value, change_column) -> None:
    ...
    conn.commit()  
    conn.close() 
    log_tg.log(3, msg=f"Задаем 0-ое значение для столбика '{change_column}', используя поисковой запрос: {search_column} -> {search_value}" + Fore.RESET)
    return